#!/usr/bin/env bash
#
# Render the Vision Transformers explainer.
#
# On first run it reuses an existing Manim venv elsewhere in the repo (the
# HarnessEngineering, Fourier or CNN series' .venv) so Manim isn't reinstalled;
# otherwise it bootstraps a local virtualenv (.venv) from requirements.txt.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | idea | patchify | embed
#             | encoder | attention | head | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (VIT_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh patchify --quick      # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export VIT_QUICK="${VIT_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) VIT_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "VisionTransformerFilm" ;;
    intro)     echo "Intro" ;;
    idea)      echo "Idea" ;;
    patchify)  echo "Patchify" ;;
    embed)     echo "Embed" ;;
    encoder)   echo "Encoder" ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$VIT_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Idea Patchify Embed Encoder Attention Head Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/VisionTransformer_${RESDIR}.mp4"
  LIST="$(mktemp)"
