#!/usr/bin/env bash
#
# Render the "What are indexes?" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | scan | index | seek
#             | composite | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (IDX_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh scan --quick        # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export IDX_QUICK="${IDX_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) IDX_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "WhatAreIndexes" ;;
    intro)     echo "Intro" ;;
    scan)      echo "Scan" ;;
    index)     echo "IndexIntro" ;;
    seek)      echo "Seek" ;;
