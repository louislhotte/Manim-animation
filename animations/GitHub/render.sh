#!/usr/bin/env bash
#
# Render the "Git & GitHub, Visually" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | overview | commit | push
#             | branch | merge | cherry | collab | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (GIT_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export GIT_QUICK="${GIT_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) GIT_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    ...
    branch)   echo "Branch" ;;
    merge)    echo "Merge" ;;
    cherry|cherrypick|cherry-pick) echo "CherryPick" ;;
    collab)   echo "Collab" ;;
    outro)    echo "Outro" ;;
    *) return 1 ;;
  esac
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$GIT_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Overview Commit Push Branch Merge CherryPick Collab Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/GitHubExplained_${RESDIR}.mp4"
  LIST="$(mktemp)"
