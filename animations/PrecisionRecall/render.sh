#!/usr/bin/env bash
#
# Render the "Precision, Recall & F1" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | matrix | precision | recall
#             | tradeoff | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (PRF_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh precision --quick     # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export PRF_QUICK="${PRF_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) PRF_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "PrecisionRecallF1" ;;
    intro)     echo "Intro" ;;
    matrix)    echo "Matrix" ;;
    precision) echo "Precision" ;;
    recall)    echo "Recall" ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$PRF_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Matrix Precision Recall Tradeoff Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/PrecisionRecallF1_${RESDIR}.mp4"
  LIST="$(mktemp)"
