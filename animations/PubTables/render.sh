#!/usr/bin/env bash
#
# Render the "PubTables-1M" paper explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | paper | tasks | scale
#             | canonical | model | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (PT_QUICK=1) for a fast test
...
#
# Media goes to /private/tmp/pt-media by default (override: PT_MEDIA_DIR) --
# rendering straight into the OneDrive-synced repo folder stalls on I/O.
#
# Examples:
#   ./render.sh paper --quick         # fast sanity check of one scene
...
MEDIA_DIR="${PT_MEDIA_DIR:-/private/tmp/pt-media}"
FILE="pubtables.py"
...
REINSTALL=0
NOCACHE=""
export PT_QUICK="${PT_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) PT_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "PubTables1M" ;;
    intro)     echo "Intro" ;;
    paper)     echo "Paper" ;;
    tasks)     echo "Tasks" ;;
    scale)     echo "Scale" ;;
    canonical) echo "Canonical" ;;
    model)     echo "ModelResults" ;;
    outro)     echo "Outro" ;;
    *) return 1 ;;
  esac
}
