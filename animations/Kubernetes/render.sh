#!/usr/bin/env bash
#
# Render the "Kubernetes & Continuous Delivery" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | why | containers | cluster
#             | replicas | pipeline | checklist | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (K8S_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export K8S_QUICK="${K8S_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) K8S_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)       echo "KubernetesCD" ;;
    intro)      echo "Intro" ;;
    why)        echo "Why" ;;
    containers) echo "Containers" ;;
    cluster)    echo "Cluster" ;;
    replicas)   echo "Replicas" ;;
    pipeline)   echo "Pipeline" ;;
    checklist)  echo "Checklist" ;;
    outro)      echo "Outro" ;;
    *) return 1 ;;
  esac
}
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$K8S_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Why Containers Cluster Replicas Pipeline Checklist Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/KubernetesCD_${RESDIR}.mp4"
  LIST="$(mktemp)"
