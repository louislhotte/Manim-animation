#!/usr/bin/env bash
#
# Render the "Semantic Layers" explainer.
#
...
# Manim venv already exists elsewhere in the repo (HarnessEngineering, Fourier
# or CNN), it is reused so Manim isn't reinstalled.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | problem | stack | whatisit
#             | usedfor | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (SEM_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export SEM_QUICK="${SEM_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) SEM_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "SemanticLayers" ;;
    intro)     echo "Intro" ;;
    problem)   echo "Problem" ;;
    stack)     echo "Stack" ;;
    whatisit)  echo "WhatIsIt" ;;
