#!/usr/bin/env bash
#
# Render the "Load Balancers" explainer.
#
...
# Manim venv already exists elsewhere in the repo (the HarnessEngineering, Gravity,
...
#
#   SCENE   full (default, the whole film) | intro | why | what | choose
#             | health | layers | scale | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (LB_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export LB_QUICK="${LB_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) LB_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)     echo "LoadBalancer" ;;
    intro)    echo "Intro" ;;
    why)      echo "Why" ;;
    what)     echo "Frontdoor" ;;
    choose)   echo "Choosing" ;;
    health)   echo "Health" ;;
    layers)   echo "Layers" ;;
    scale)    echo "Scale" ;;
    outro)    echo "Outro" ;;
