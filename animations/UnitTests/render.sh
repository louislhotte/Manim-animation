#!/usr/bin/env bash
#
# Render the Unit Tests explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | unit | anatomy | assertion
#             | edges | net | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (UT_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh anatomy --quick        # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export UT_QUICK="${UT_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) UT_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "UnitTestsFilm" ;;
    intro)     echo "Intro" ;;
    unit)      echo "Unit" ;;
    anatomy)   echo "Anatomy" ;;
    assertion) echo "Assertion" ;;
    edges)     echo "Edges" ;;
    net)       echo "Net" ;;
    recap)     echo "Recap" ;;
    outro)     echo "Outro" ;;
