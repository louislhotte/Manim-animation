#!/usr/bin/env bash
#
# Render the "Role-Based Access Control" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | tangle | roles | check
#             | recap | outro
...
#
# Examples:
#   ./render.sh roles --quick          # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export RBAC_QUICK="${RBAC_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) RBAC_QUICK=1; shift ;;
