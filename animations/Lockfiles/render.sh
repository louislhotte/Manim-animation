#!/usr/bin/env bash
#
# Render the Lockfiles explainer (package.json vs package-lock.json).
#
...
#
#   SCENE   full (default, the whole film) | intro | manifest | ranges | lockfile
#             | repro | pattern | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (PL_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh ranges --quick         # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export PL_QUICK="${PL_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) PL_QUICK=1; shift ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$PL_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Manifest Ranges Lockfile Repro Pattern Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/LockfilesFilm_${RESDIR}.mp4"
  LIST="$(mktemp)"
