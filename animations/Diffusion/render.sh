#!/usr/bin/env bash
#
# Render the "Diffusion Models, Visually" explainer.
#
...
# Fourier series' .venv), it is reused so Manim isn't reinstalled. It also bakes
# assets/diffusion.npz (the noise<->image trajectories) if it is missing.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | hook | examples | forward
#             | reverse | steps | prompt | comfy | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (DIFF_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh forward --quick       # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export DIFF_QUICK="${DIFF_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) DIFF_QUICK=1; shift ;;
...
# ---- bake trajectories if missing ----------------------------------------- #
if [ ! -f "$ROOT/assets/diffusion.npz" ]; then
  echo ">> Baking assets/diffusion.npz"
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$DIFF_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Hook Examples Forward Reverse Steps Prompt Comfy Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/DiffusionModels_${RESDIR}.mp4"
  LIST="$(mktemp)"
