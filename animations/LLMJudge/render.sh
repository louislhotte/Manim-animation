#!/usr/bin/env bash
#
# Render the "LLM-as-a-Judge" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | gap | idea | prompt
#             | evaluators | pipeline | limits | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (LLMJ_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export LLMJ_QUICK="${LLMJ_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) LLMJ_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)       echo "LLMAsJudge" ;;
    intro)      echo "Intro" ;;
    gap)        echo "Gap" ;;
    idea)       echo "Idea" ;;
    prompt)     echo "Prompt" ;;
    evaluators) echo "Evaluators" ;;
