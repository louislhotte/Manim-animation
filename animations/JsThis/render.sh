#!/usr/bin/env bash
#
# Render the JavaScript `this` explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | hook | dot | lost | rules
#             | fix | takeaway | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (THIS_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh dot --quick             # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export THIS_QUICK="${THIS_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) THIS_QUICK=1; shift ;;
