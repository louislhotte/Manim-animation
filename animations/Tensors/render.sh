#!/usr/bin/env bash
#
# Render the "What is a Tensor?" explainer.
#
...
#
#   SCENE   full (default, the whole film) | hook | intro | scalar | vector
#             | matrix | higher | data | compute | recap | outro | probe
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (TN_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh vector --quick        # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export TN_QUICK="${TN_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) TN_QUICK=1; shift ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$TN_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Hook Intro Scalar Vector Matrix Higher DataUse Compute Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/WhatIsATensor_${RESDIR}.mp4"
  LIST="$(mktemp)"
