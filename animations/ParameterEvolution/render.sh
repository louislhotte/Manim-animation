#!/usr/bin/env bash
#
# Render the Parameter-Evolution explainer.
#
...
#
#   SCENE   full (default, the whole ~3-min film) | intro | setup | descent
#             | cvlinear | manyknobs | trainnet | cvnet | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Collapse the on-screen holds (PE_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh descent --quick       # fast sanity check of the gradient-descent scene
...
REINSTALL=0
NOCACHE=""
export PE_QUICK="${PE_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) PE_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "ParameterEvolution" ;;
    intro)     echo "Intro" ;;
    setup)     echo "Setup" ;;
    descent)   echo "Descent" ;;
    cvlinear)  echo "CVLinear" ;;
    manyknobs) echo "ManyKnobs" ;;
    trainnet)  echo "TrainNet" ;;
    cvnet)     echo "CVNet" ;;
    recap)     echo "Recap" ;;
    outro)     echo "Outro" ;;
    *) return 1 ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$PE_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Setup Descent CVLinear ManyKnobs TrainNet CVNet Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/ParameterEvolution_${RESDIR}.mp4"
  LIST="$(mktemp)"
