#!/usr/bin/env bash
#
# Render the Cross-Validation demo.
#
...
#
#   SCENE   full (default, the whole ~1-min film) | intro | overfit | kfold
#             | payoff | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (CV_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh overfit --quick       # fast sanity check of the overfitting scene
...
REINSTALL=0
NOCACHE=""
export CV_QUICK="${CV_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) CV_QUICK=1; shift ;;
