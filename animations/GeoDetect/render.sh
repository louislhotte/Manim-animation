#!/usr/bin/env bash
#
# Render the "Detecting Change From Space" geospatial-detection explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | signal | deforestation
#             | agriculture | recap | outro | probe
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (GD_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh signal --quick        # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export GD_QUICK="${GD_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) GD_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)          echo "DetectingChangeFromSpace" ;;
    intro)         echo "Intro" ;;
    signal)        echo "Signal" ;;
    deforestation) echo "Deforestation" ;;
    agriculture)   echo "Agriculture" ;;
    recap)         echo "Recap" ;;
    outro)         echo "Outro" ;;
    probe)         echo "Probe" ;;
    *) return 1 ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$GD_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Signal Deforestation Agriculture Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/DetectingChangeFromSpace_${RESDIR}.mp4"
  LIST="$(mktemp)"
