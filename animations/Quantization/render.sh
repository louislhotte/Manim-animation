#!/usr/bin/env bash
#
# Render the "Model Quantization" explainer (a 3D, perpetually-orbiting piece).
#
...
# otherwise it bootstraps a local .venv from requirements.txt.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | network | numbers | snap
#             | compare | mapping | payoff | outro | probe
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (QZ_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh field --quick        # fast sanity check of one scene
...
# OneDrive I/O. Override with QZ_MEDIA_DIR=... (or point it at "$ROOT/media").
MEDIA_DIR="${QZ_MEDIA_DIR:-/private/tmp/qz-media}"
...
REINSTALL=0
NOCACHE=""
export QZ_QUICK="${QZ_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) QZ_QUICK=1; shift ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$QZ_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Network Numbers Snap Compare Mapping Payoff Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/ModelQuantization_${RESDIR}.mp4"
  LIST="$(mktemp)"
