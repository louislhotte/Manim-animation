#!/usr/bin/env bash
#
# Render "The Immortal Game" chess replay (Anderssen vs. Kieseritzky, 1851).
#
...
# Manim venv already exists elsewhere in the repo (HarnessEngineering, Gravity,
# Fourier or CNN), it is reused so Manim isn't reinstalled.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--plies N] [--stitch]
#               [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | game | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the reading holds (CHESS_QUICK=1) for a fast test
#   --plies N      Stop the game after N half-moves (dev aid)
#   --stitch       Render intro + game + outro and join them into one film
#   --no-cache     Force a full re-render
#   --reinstall    Recreate the local venv from scratch
#
# Examples:
#   ./render.sh game --quick --plies 10   # fast sanity check of the opening
#   ./render.sh                           # the whole film, 480p
#   ./render.sh full -q m                 # final 720p render
#   ./render.sh --stitch -q m             # render each part and stitch (720p)
#
set -euo pipefail
...
REINSTALL=0
NOCACHE=""
export CHESS_QUICK="${CHESS_QUICK:-0}"
export CHESS_PLIES="${CHESS_PLIES:-999}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) CHESS_QUICK=1; shift ;;
    --plies) CHESS_PLIES="${2:?--plies needs a number}"; shift 2 ;;
...
if [ -x "$VENV/bin/python" ]; then
  ...
  for CAND in "$HARNESS_VENV" "$GRAVITY_VENV" "$FOURIER_VENV" "$CNN_VENV"; do
...
export CHESS_QUICK CHESS_PLIES

# ---- render --------------------------------------------------------------- #
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$CHESS_QUICK, plies=$CHESS_PLIES)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Game Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/Immortal_${RESDIR}.mp4"
  LIST="$(mktemp)"
