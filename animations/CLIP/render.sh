#!/usr/bin/env bash
#
# Render the "What is CLIP?" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | problem | encoders
#             | contrast | zeroshot | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (CLIP_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh contrast --quick      # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export CLIP_QUICK="${CLIP_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) CLIP_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "WhatIsCLIP" ;;
    intro)     echo "Intro" ;;
    problem)   echo "Problem" ;;
    encoders)  echo "Encoders" ;;
    contrast)  echo "Contrast" ;;
    zeroshot)  echo "ZeroShot" ;;
    outro)     echo "Outro" ;;
    *) return 1 ;;
  esac
}
