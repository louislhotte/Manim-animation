#!/usr/bin/env bash
#
# Render the "React Hooks & useEffect" explainer.
#
...
#
#   SCENE   full (default, the whole ~3-min film) | intro | hooks | state
#             | effects | anatomy | example | cycle | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Collapse the on-screen holds (RX_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh example --quick       # fast sanity check of the star scene
...
REINSTALL=0
NOCACHE=""
export RX_QUICK="${RX_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) RX_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)     echo "ReactHooks" ;;
    intro)    echo "Intro" ;;
    hooks)    echo "Hooks" ;;
    state)    echo "State" ;;
    effects)  echo "SideEffects" ;;
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$RX_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Hooks State SideEffects Anatomy Example Cycle Recap Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/ReactHooks_${RESDIR}.mp4"
  LIST="$(mktemp)"
