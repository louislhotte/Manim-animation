#!/usr/bin/env bash
#
# Render the "What is a GAN?" explainer (a dynamic 3-D, orbiting piece).
#
# On first run it reuses an existing Manim venv elsewhere in the repo (the
# HarnessEngineering / Fourier / CNN series' .venv) so Manim isn't reinstalled;
# otherwise it bootstraps a local .venv from requirements.txt. It also bakes the
# real toy-GAN assets (assets/gan.npz) on first run if they are missing.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--assets] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | goal | players | generator
#             | discriminator | game | convergence | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (GAN_QUICK=1) for a fast test
#   --stitch       Render every section scene and join them into one film
#   --assets       Force a re-bake of assets/gan.npz
#   --no-cache     Force a full re-render
#   --reinstall    Recreate the local venv from scratch
#
# Examples:
#   ./render.sh discriminator --quick   # fast sanity check of one scene
...
# Render into a local scratch dir by default: this repo lives on a OneDrive-synced
# path, and writing Manim's many partial-movie files there stalls for minutes on
# OneDrive I/O. Override with GAN_MEDIA_DIR=... (or point it at "$ROOT/media").
MEDIA_DIR="${GAN_MEDIA_DIR:-/private/tmp/gan-media}"
FILE="gan.py"
...
ASSETS=0
NOCACHE=""
export GAN_QUICK="${GAN_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) GAN_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)          echo "WhatIsAGAN" ;;
    intro)         echo "Intro" ;;
    goal)          echo "Goal" ;;
    players)       echo "Players" ;;
    generator)     echo "Generator" ;;
    discriminator) echo "Discriminator" ;;
    game)          echo "Game" ;;
    convergence)   echo "Convergence" ;;
    outro)         echo "Outro" ;;
    *) return 1 ;;
  esac
}
...
# ---- bake the real GAN assets if needed ----------------------------------- #
if [ "$ASSETS" -eq 1 ] || [ ! -f "$ROOT/assets/gan.npz" ]; then
  echo ">> Baking real toy-GAN assets (assets/gan.npz)"
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$GAN_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro Goal Players Generator Discriminator Game Convergence Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/WhatIsAGAN_${RESDIR}.mp4"
  LIST="$(mktemp)"
