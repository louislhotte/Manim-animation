#!/usr/bin/env bash
#
# Render the Docling explainer (chunking + OCR).
#
...
#
#   SCENE   full (default, the whole film) | intro | problem | pipeline | layout
#             | ocr | chunk | recap | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (DL_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh ocr --quick             # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export DL_QUICK="${DL_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) DL_QUICK=1; shift ;;
