#!/usr/bin/env bash
#
# Render the "Content Delivery Networks" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | problem | idea
#             | how | use | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (CDN_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export CDN_QUICK="${CDN_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) CDN_QUICK=1; shift ;;
