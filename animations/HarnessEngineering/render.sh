#!/usr/bin/env bash
#
# Render the Harness Engineering explainer.
#
...
# If a Manim venv already exists elsewhere in the repo (the CNN or Fourier
# series' .venv), it is reused automatically so Manim isn't reinstalled.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film)  | intro | what | prompt | context
#             | harness | pipeline | why | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (HARNESS_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh what --quick          # fast sanity check of scene 1
...
REINSTALL=0
NOCACHE=""
export HARNESS_QUICK="${HARNESS_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) HARNESS_QUICK=1; shift ;;
...
export HARNESS_QUICK

# ---- render --------------------------------------------------------------- #
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$HARNESS_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro What Prompt Context Harness Pipeline Why Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/HarnessEngineering_${RESDIR}.mp4"
  LIST="$(mktemp)"
