#!/usr/bin/env bash
#
# Render the "Geometry of (a+b)²" explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | hook | proof | vary
#             | grid | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (PS_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export PS_QUICK="${PS_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) PS_QUICK=1; shift ;;
