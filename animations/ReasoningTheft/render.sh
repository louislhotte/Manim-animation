#!/usr/bin/env bash
#
# Render the "Stealing a Model's Reasoning" explainer.
#
...
# Fourier or CNN series' .venv), it is reused so Manim isn't reinstalled.
#
# Usage:
#   ./render.sh [SCENE] [-q l|m|h|k] [-p] [--quick] [--stitch] [--no-cache] [--reinstall]
#
#   SCENE   full (default, the whole film) | intro | cot | lockbox | onekey
#             | bankshot | harvest | distill | fix | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (RT_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh bankshot --quick       # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export RT_QUICK="${RT_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) RT_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)     echo "StealingReasoning" ;;
    intro)    echo "Intro" ;;
    cot)      echo "CoT" ;;
    lockbox)  echo "Lockbox" ;;
    onekey)   echo "OneKey" ;;
    bankshot) echo "BankShot" ;;
    harvest)  echo "Harvest" ;;
    distill)  echo "Distill" ;;
    fix)      echo "Fix" ;;
    outro)    echo "Outro" ;;
    *) return 1 ;;
  esac
...
render_one() {
  local klass="$1"
  echo ""
  echo ">> Rendering $klass  ($QFLAG, quick=$RT_QUICK)"
  ( cd "$ROOT" && "$PY" -m manim $QFLAG $PREVIEW $NOCACHE --media_dir "$MEDIA_DIR" "$FILE" "$klass" )
}

if [ "$STITCH" -eq 1 ]; then
  ORDER=(Intro CoT Lockbox OneKey BankShot Harvest Distill Fix Outro)
  OUTPUTS=()
  for klass in "${ORDER[@]}"; do
    render_one "$klass"
    OUTPUTS+=("$MEDIA_DIR/videos/${FILE%.py}/$RESDIR/$klass.mp4")
  done
  FULL="$MEDIA_DIR/StealingReasoning_${RESDIR}.mp4"
  LIST="$(mktemp)"
