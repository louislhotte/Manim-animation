#!/usr/bin/env bash
#
# Render the AI Guardrails explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | threat | idea | input
#             | output | catch | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (GR_QUICK=1) for a fast test
...
#
# Examples:
#   ./render.sh input --quick        # fast sanity check of one scene
...
REINSTALL=0
NOCACHE=""
export GR_QUICK="${GR_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) GR_QUICK=1; shift ;;
