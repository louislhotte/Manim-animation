#!/usr/bin/env bash
#
# Render the Bias–Variance Tradeoff explainer.
#
...
#
#   SCENE   full (default, the whole film) | intro | setup | underfit
#             | justright | overfit | tradeoff | outro
#   -q      Quality: l=480p15 (default, fast), m=720p30, h=1080p60, k=2160p60
#   -p      Preview: open the clip when it finishes
#   --quick        Shorten the on-screen holds (BV_QUICK=1) for a fast test
...
REINSTALL=0
NOCACHE=""
export BV_QUICK="${BV_QUICK:-0}"

while [ $# -gt 0 ]; do
  case "$1" in
    -q) QUALITY="${2:?-q needs a value: l|m|h|k}"; shift 2 ;;
    -p) PREVIEW="-p"; shift ;;
    --quick) BV_QUICK=1; shift ;;
...
scene_class() {
  case "$1" in
    full)      echo "BiasVariance" ;;
    intro)     echo "Intro" ;;
    setup)     echo "Setup" ;;
    underfit)  echo "Underfit" ;;
    justright) echo "JustRight" ;;
    overfit)   echo "Overfit" ;;
    tradeoff)  echo "Tradeoff" ;;
    outro)     echo "Outro" ;;
    *) return 1 ;;
  esac
}
